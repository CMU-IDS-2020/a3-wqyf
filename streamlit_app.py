...
def load_geo(data, lon, lat, radius):
    data['dist'] = list(
        map(lambda k: get_distance(data.loc[k]['pickup_longitude'], data.loc[k]['pickup_latitude'], lon, lat),
            data.index))
...
st.write("Select time range of pickup")
...
chart_data = pd.DataFrame({"hour": range(25), "pickups": hist})
chart_data['start'] = start_hour
chart_data['end'] = end_hour
chart1 = alt.Chart(chart_data).mark_area(interpolate='step-after',)\
    .encode(x=alt.X("hour:O", scale=alt.Scale(nice=False)),y=alt.Y("pickups:Q"),tooltip=['hour', 'pickups'])\
...
animations = {"None": None, "Slow": 3, "Fast": 1}
animate = st.sidebar.radio("Animation Choices", options=list(animations.keys()), index=0)
...
if animation_speed:
    ...
    hour_text.write("**Between " + str(start_time.time()) + " and " + str(end_time.time()) + "**")
    fmap(data, midpoint[0], midpoint[1], zoom_level, layer_names)
